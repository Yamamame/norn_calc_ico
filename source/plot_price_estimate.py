...
table_get_col = {"symbols": 0, "time": 1, "min": 2, "max": 3, "close": 5}
...
span_end = datetime.date.today() - datetime.timedelta(days=30)
span_start = span_end - datetime.timedelta(days=int(30 * 3))

# 30分毎なので47で1日 0:00と24:00がおなじになるように
time_ago = (span_start - span_end).min / 30
# 評価パラメータをいくつ用いるか？
num_param = 1
...
candle_data = db_access.get_recent_period_tith_datetime(
    instrument=instrument_strs[0], span_end=span_end, span_start=span_start)
...
# minデータをYに入れる
for i in range(0, (time_ago - 1)):
    D[i] = abs(targ_data[i, data_kind_open] - targ_data[i, table_get_col["close"]]) * 50
